...
function initHistory(): [number, number][] {
...
export function useElo() {
  ...
  const [history, setHistory] = useState<[number, number][]>(initHistory);

  const vote = useCallback((winnerId: number | null, loserId: number | null) => {
    if (winnerId === null || loserId === null) {
      // Draw — record the duel as played but no score change
      setHistory(prev => {
        const next = [...prev, [winnerId ?? -1, loserId ?? -1] as [number, number]];
  ...
  const getNextDuel = useCallback((): readonly [Player, Player] => {
    ...
    // Fallback: any two not in recent
  ...
  const reset = () => {
    const fresh = Object.fromEntries(players.map(p => [p.id, 1500]));
    setScores(fresh);
    setHistory([]);
  ...
  return { scores, vote, getNextDuel, ranking, comparisons: history.length, reset, duelsPlayed, players };
}
